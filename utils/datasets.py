...
#这个用来多batch_size -detect的 把原图都变成同一个img_size
class LoadImagesEn(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        print('image %g/%g %s: ' % (index, self.nf, path), end='')

        # Padded resize
        img = letterbox(img0, new_shape=self.img_size)[0]
        if img.shape[0] != img.shape[1]:#,'image  weight height no !!, {}'.format(img.shape)
