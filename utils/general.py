...
import pdb
from utils.kld_loss import KLDloss
...
def compute_loss_kld(p,targets,model):
    ...
    '''
        ...
        tangle : 3个tensor组成的list (tensor_angle_list[i])  对每个步长网络生成对应的class tensor
                       tangle[i].shape=(num_i)
    '''
    tcls, tbox, indices, anchors= build_targets_kld(p, targets, model)  # targets
    #tcls, tbox, indices, anchors, tangle = build_targets(p, targets, model)  # targets
...
def rotate_non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False, without_iouthres=False,kld_flag=False):
    """
...
def plot_results(start=0, stop=0, bucket='', id=(), labels=(), save_dir=''):
    ...
    ax = ax.ravel()
    s = ['Box', 'Objectness', 'Classification', 'Angle','Precision', 'Recall','mAP@0.5', 'mAP@0.5:0.95',
         'val Box', 'val Objectness', 'val Classification','val Angle' ]
    if bucket:
    ...
    for fi, f in enumerate(files):
        try:
            results = np.loadtxt(f, usecols=[2, 3, 4, 5, 9, 10, 11, 12, 13, 14,15, 16], ndmin=2).T
            n = results.shape[1]  # number of rows
            x = range(start, min(stop, n) if stop else n)
            for i in range(12):
                y = results[i, x]
                if i in [2, 3, 4, 5, 9, 10, 11, 12, 13, 14, 15,16]:
